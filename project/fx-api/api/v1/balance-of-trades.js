...
exports.bootstrap = (dependencies) => {
    ...
    app.get(endpoint, (req, res) => {
        ...
        (async () => {
            ...
            await page.goto(`https://tradingeconomics.com/${target}/balance-of-trade`);
            try {
                await page.waitForSelector("table#calendar");
                console.log(`BoT - request OK - query target ${target}`);
            } catch (ex) {
                res.status(404).send({
                    result: "invalid target"
                })

                console.log(`BoT - request NOT OK - query target ${target}`);
                return;
            }

            console.log(`BoT - webcrawling... - query target ${target}`);
            ...
            console.log(`BoT - webcrawling finished - query target ${target}`);
